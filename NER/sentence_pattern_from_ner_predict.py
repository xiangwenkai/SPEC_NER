...
from seperate_spec import sentence_split, insertSeparator
...
def get_sentence_embedding(sentence_list):
    ...
    for s in sentence_list:
        ...
        with torch.no_grad():
            ...
            emb = list(sentence_embeddings.detach().cpu().numpy()[0].astype(str))
...
def specPatternAnalysis(file,targetDomain,k=40):
    """
    file: .xlsx
    targetDomain: "AE"
    """
    #  读入spec excel 文件
    df_all,_ = read_xls(file)

    spec_list = df_all['Conversion Definition'].tolist()
    spec_list = [sample_preprocess(x) for x in spec_list]

    # 预测spec
    ner_predict_list = ner_predict(model_path='AZ/model-best', spec_list=spec_list)

    # 实体替换
    df_all['rep'] = get_ents_rep(raws, ner_predict_list)
    ...
    clusters = get_cluster(embedding_list)
...
# 获取句子embeddings
embeddings = get_sentence_embedding(data['sentence'].tolist()[:10])
